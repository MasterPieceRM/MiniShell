...
static char *get_value_env(word_t *aux)
{
	...
	if (aux->expand == true) {
		...
		memcpy(value, getenv(aux->string), strlen(getenv(aux->string)));
		value[strlen(getenv(aux->string))] = '\0';
	} else {
		...
		memcpy(value, aux->string, strlen(aux->string));
		value[strlen(aux->string)] = '\0';
	}
	aux = aux->next_part;
	while (aux) {
		if (aux->expand == true) {
...
static int parse_simple(simple_command_t *s, int level, command_t *father)
{
	...
	if (s->verb->next_part != NULL && s->verb->next_part->string[0] == '=') {
		...
		memcpy(var, s->verb->string, strlen(s->verb->string));
		var[strlen(s->verb->string)] = '\0';
	...
	if (pid == -1) {
		...
		memcpy(args[index], s->verb->string, strlen(s->verb->string));
		args[index][strlen(s->verb->string)] = '\0';
		index++;
		while (aux->params) {
			...
			if (aux->params->expand == true) {
				...
				memcpy(args[index], getenv(aux->params->string), strlen(getenv(aux->params->string)));
				args[index][strlen(getenv(aux->params->string))] = '\0';
				...
				memcpy(args[index], aux->params->string + offset, strlen(aux->params->string) + offset);
				args[index][strlen(aux->params->string) + offset] = '\0';
			}
			index++;
